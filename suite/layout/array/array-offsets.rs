...
fn check_array_elem_offsets<T>() {
    let x = [const { unsafe { core::mem::zeroed::<T>() } }; 32];

    let addr = (&raw const x).cast::<T>();

    let end = unsafe { (&raw const x).add(1).cast::<T>() }; // Add single array
...
fn test_array_offsets() {
    check_array_elem_offsets::<u8>();
    check_array_elem_offsets::<u16>();
    check_array_elem_offsets::<u64>();
    check_array_elem_offsets::<[u8; 63]>();
    check_array_elem_offsets::<[u16; 17]>();
}
